...
import { loadEmployees } from './modules/init.js'; // Import the loadEmployees function
...
async function init() {
    try {
        const employeeData = await loadEmployees(); // Fetch the employee data
        if (employeeData && employeeData.employees) {
            buildGrid(employeeData.employees); // Pass the employees array to buildGrid
        }
    } catch (error) {
...
function buildGrid(arrEmployees) {
    ...
    for (let employee of arrEmployees) {
        ...
        `
       <tr>
            <td>${employee.id}</td> <!-- ID -->
            <td>${employee.name}</td> <!-- Name -->
            <td>${employee.ext}</td> <!-- Extension -->
            <td><a href="mailto:${employee.email}">${employee.email}</a></td> <!-- Email -->
            <td>${employee.department}</td> <!-- Department -->
